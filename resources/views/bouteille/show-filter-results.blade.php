        ...
        <main class="nav-margin">
            <section class="form-ajouter-bouteille"> <!-- encadré noir (formulaires, filtres et tris) --> 
                <div class="form-container">
                    <form action="{{ route('bouteille.search') }}" method="GET" id="">
                        <!-- @csrf -->
            ...
            <div class="card-count">
             <p>{{$fbouteilles->total()}} bouteilles :</p>
            </div>
            @foreach ($fbouteilles as $fbouteille)
            <section class="card-bouteille">
                <picture>
                    <img src="{{ $fbouteille->srcImage }}" alt="{{ $fbouteille->nom}}">
                </picture>
                <div class="card-bouteille-content">
                    <div class="card-bouteille-info">
                        <h2><a href="{{ route('bouteille.show',['bouteille_id'=> $fbouteille->id]) }}">{{ $fbouteille->nom }}</a></h2>
                        <span>{{$fbouteille->type}} | {{ $fbouteille->format }} | {{$fbouteille->pays}}</span>
                        <p>{{$fbouteille->prix}} $</p>
                    </div>
            ...
            @endforeach  
             {{ $fbouteilles->appends(request()->query())->links('pagination.custom') }} 
        </section>
    </main>
    @endsection
